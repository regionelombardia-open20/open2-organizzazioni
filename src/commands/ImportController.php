...
class ImportController extends Controller
{
        ...
        if (!empty($notImportedOrganizations)) {
            foreach ($notImportedOrganizations as $k => $organizationToImport) {
                ...
                foreach ($requiredFields as $requiredField) {
                    if (!isset($organizationToImport[$requiredField]) || empty($organizationToImport[$requiredField])) {
                        $checkRequired = false;
                    }
                }
                if (!$checkRequired) {
                    MigrationCommon::printCheckStructureError($organizationToImport, Module::t('amosorganizzazioni', '#import_organizations_missing_required_field'));
                    continue;
                }
                if (!empty($organizationToImport[$keyForImport])) {
                    $organization = $this->findOrganizationByImportKey($organizationToImport);
                    if (!empty($organization)) {
                        MigrationCommon::printConsoleMessage(Module::t('amosorganizzazioni', '#import_organizations_already_present', [
                    ...
                    $ok = $this->beforeLoadOrganizationValues($organization, $organizationToImport);
                    if (!$ok) {
                        MigrationCommon::printCheckStructureError($organizationToImport, Module::t('amosorganizzazioni', '#import_organizations_before_load_error'));
                        continue;
                    }
                    $ok = $this->loadOrganizationValues($organization, $organizationToImport);
                    if (!$ok) {
                        MigrationCommon::printCheckStructureError($organizationToImport, Module::t('amosorganizzazioni', '#import_organizations_load_error'));
                        continue;
                    }
                    $ok = $this->afterLoadOrganizationValues($organization, $organizationToImport);
                    if (!$ok) {
                        MigrationCommon::printCheckStructureError($organizationToImport, Module::t('amosorganizzazioni', '#import_organizations_after_load_error'));
                        continue;
                    }
                    ...
                    if ($organization->save(false)) {
                        ...
                        $this->beforeCreateMainHeadquarters($organization, $organizationToImport);
    ...
    protected function findOrganizationByImportKey($organizationToImport)
    {
        ...
        /** @var ProfiloImport $organization */
        $organization = $query->andWhere([
            $keyForImport => $organizationToImport[$keyForImport]
        ])->one();
    ...
    {
        $geocodeResponse = new OrganizationsPlaces();
