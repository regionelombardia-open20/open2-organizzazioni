...
JS;
    ...
    public function run()
    {
        ...
        if ($place_fake) {
            $geocodeString = OrganizationsPlacesComponents::getGeocodeString($placeDataObj);
            if ($geocodeString) {
                $tmp_place_id = OrganizationsPlacesComponents::getGoogleResponseByGeocodeString($geocodeString, true);
                if ($tmp_place_id) {
